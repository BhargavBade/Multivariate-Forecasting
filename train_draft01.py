...
# Get the tensors
(train_data_tensor, train_labels_tensor, val_data_tensor, val_labels_tensor, past_pm25_val, 
 test_data_tensor, test_labels_tensor, past_pm25_test, scaler, pm_index) = data_preparer.get_tensors()
...
# In[52]:
#Evaluation of the Valdiation Set
...
# Step 2: Reverse Standardization for Validation set Predictions
means = scaler.mean_
stds = scaler.scale_
...
# Plotting All VAL predictions at a time
...
# # Plotting test predicitions as sequences.
...
#     plt.plot(range(sequence_length), test_actual_labels_original_scale[start_idx:end_idx], label=f'Actual PM2.5 (Seq {i+1})', alpha=0.7)
#     plt.plot(range(sequence_length), test_predictions_original_scale[start_idx:end_idx], label=f'Predicted PM2.5 (Seq {i+1})', alpha=0.7)
...
## Plotting test predicitions as sequences for multi-step predictor.
...
for i, start_idx in enumerate(random_indices):
    ...
    # Plot the actual PM2.5 values (single ground truth)
    plt.plot(range(sequence_length), test_actual_labels_original_scale[start_idx, :sequence_length], label= 'Actual PM2.5 (Ground Truth)', color='blue', alpha=0.7)
    
    # Plot predictions for each future step with different colors
    for step in range(future_steps):
        plt.plot(range(sequence_length), test_predictions_original_scale[start_idx, :sequence_length], label=f'Predicted PM2.5 (Step {step+1})', color=prediction_colors[step], alpha=0.7)
    ...
    plot_filename = f'predicted_vs_actual_multistep_sequence_{i+1}.png'
