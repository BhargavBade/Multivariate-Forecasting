...
import params_sparse_data
importlib.reload(params_sparse_data)

# In[36]:

print(params_sparse_data.seq_len)
...
wandb_name = params_sparse_data.wandb_name
wandb_project = params_sparse_data.wandb_project

wandb_model_name = wandb_name + f'_multivar_{current_time}' + \
            '_bs' + str(params_sparse_data.batch_size) + '_lr' + str(params_sparse_data.lr) + '_ep' + \
            str(params_sparse_data.num_epochs)  +'.pt'
                      
# In[40]:

import sys
sys.path.append('./Data')  # Add the current directory to Python path
import prepare_data_sparse
importlib.reload(prepare_data_sparse)
from prepare_data_sparse import DataPreparer
...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=params_sparse_data.batch_size,
...
# Create Train DataLoader
val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=params_sparse_data.batch_size,
                                           shuffle=False, drop_last=True)
...
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=params_sparse_data.batch_size,
                                           shuffle=False, drop_last=True)

#----------------------------------------------------------------------------------------------

seq_len = params_sparse_data.seq_len
label_len = params_sparse_data.label_len
pred_len = params_sparse_data.pred_len
...
enc_inp = params_sparse_data.enc_inp  # number of features
dec_inp = params_sparse_data.dec_in 
c_out = params_sparse_data.c_out  # output size (1 for regression, could be different for classification)
...
optimizer = optim.Adam(model.parameters(), lr = params_sparse_data.lr)
...
wandb.init(  
      ...
      config={
        "epochs": params_sparse_data.num_epochs,
        "bs": params_sparse_data.batch_size,
        "lr": params_sparse_data.lr,
        "lookback Horz": params_sparse_data.seq_len,
        "label len": params_sparse_data.label_len,
        })
...
num_epochs = params_sparse_data.num_epochs
patience = params_sparse_data.early_stop_patience  # Number of epochs to wait before stopping if no improvement
...
warmup_epochs = params_sparse_data.warmup_epochs 
...
num_features = params_sparse_data.enc_inp
...
test_actual_concat = test_actual_labels_original_scale.reshape(-1, params_sparse_data.enc_inp)  
test_pred_concat = test_predictions_original_scale.reshape(-1, params_sparse_data.enc_inp)

# Plot each feature separately
for feature_idx in range(params_sparse_data.enc_inp):
...
# Define parameters
num_features = params_sparse_data.enc_inp  # Total number of features
num_samples_to_plot = params_sparse_data.num_samples_to_plot # Number of random samples to plot
...
selected_feature_names = params_sparse_data.feat_for_plotting
...
for sample_idx in selected_samples:
    ...
    past_values = test_pastvalues_org_scale[sample_idx]  # Shape: (365, 10)
    actual_values = test_actual_labels_original_scale[sample_idx]  # Shape: (180, 10)
    predicted_values = test_predictions_original_scale[sample_idx]  # Shape: (180, 10)

    # Insert the first actual value at index 91 in past_values
    past_values_extended = np.vstack((past_values[:params_sparse_data.label_len, :], actual_values[0, :], past_values[params_sparse_data.label_len:, :]))  # Shape: (366, 10)
    
    # Extract past_values_modified (0 to 91 indices)
    past_values_modified = past_values_extended[:params_sparse_data.label_len+1, :]  # Now exactly (186, 10)
    ...
    for feature_idx, feature_name in zip(selected_feature_indices, selected_feature_names):
        ...
        time_steps_past = test_dec_org_datestamp[sample_idx, 1:params_sparse_data.label_len+1, 0]  # First 90 timestamps       
        ...
        if params_sparse_data.nan_fraction > 0.75:
