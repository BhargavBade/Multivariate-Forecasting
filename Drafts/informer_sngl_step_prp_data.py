...
class DataPreparer:
    ...
    """
        ...
        for i in range(len(input_sequences) - seq_len - pred_len + 1):
            # Encoder input
            seq_x = input_sequences.iloc[i:i + seq_len, :].values
            
            # Target output
            seq_y = output_sequence.iloc[i + seq_len:i + seq_len + pred_len].values
            
            # Extract PM2.5 values for the input sequence (aligned with the output)
            pm25_past_values = output_sequence[i:i + seq_len+1].values  # PM2.5 values in the past window
            ...
            dec_inp[:label_len] = output_sequence.iloc[i + seq_len - label_len:i + seq_len].values.reshape(-1, 1)
            
           # TimeStamp data for the encoder and decoder
            enc_time = data_stamp[i:i + seq_len]
            dec_time = data_stamp[i + seq_len - label_len:i + seq_len + pred_len]
            #--------------------------------------------------------------------------------
    ...
    def prepare_data(self):
        ...
        self.val_data_tensor = torch.Tensor(X_test).float()
        self.val_labels_tensor = torch.Tensor(y_test).float()
        self.val_mark_tns = torch.Tensor(X_test_datestamp).float()
        self.val_labl_mark_tns = torch.Tensor(y_test_datestamp).float()
        self.val_output_gt = torch.Tensor(output_y_test).float()
        self.val_past_pm = torch.Tensor(past_pm_test).float()
